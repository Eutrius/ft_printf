...
/*   Updated: 2024/12/29 00:31:18 by jyriarte         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	while (str[*index] != '\0' && ft_strchr(FORMAT_FLAGS, str[*index]))
	{
		ft_strcat(format->flags, str[*index]);
		(*index)++;
	}
	...
	if (str[*index] && ft_strchr(FORMAT_SPECIFIERS, str[*index]))
		return (1);
	return (0);
}
